...
class MyFSR:
    ...
    @staticmethod
    def metric_fsr(my_array, m, n, columns_nominal=np.array([])):
        fsr = np.zeros((m, n, n))
        for k in range(m):
    ...
    def calculate_metric_fsr(self, columns_nominal=np.array([])):
        self.metric_fsr_value = self.metric_fsr(self.my_array, self.m, self.n, columns_nominal)
...
def reduce_mfren(columns, metric_fsr, fsr_obj, lambda_v=0.0):
    zero_red = []
